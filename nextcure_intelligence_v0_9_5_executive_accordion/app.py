...
from ui.layout import render_hero, render_insights, render_kpi_cards, render_watch_items, render_dashboard_nav, render_synthesis_summary, render_premium_executive_summary
...
with left:
    st.markdown(
        ...
        <div class="card">
            ...
            <div class="muted">
                No tuning panel, no analyst-style clutter. Leadership opens the dashboard, presses START ANALYSIS,
...
if st.session_state.analysis_started and st.session_state.results is not None:
    ...
    if active_page == "Executive Summary":
        if getattr(results, "using_real_data", False):
            st.markdown('<div class="exec-data-note good">Live market data active for this run.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="exec-data-note warn">Prototype fallback data is showing because live benchmark data was unavailable. Fresh-intelligence scaffolding remains active.</div>', unsafe_allow_html=True)
    ...
    if active_page == "Interpretation Engine":
        ...
        st.markdown(
            ...
            <div class="muted" style="margin-bottom: .85rem;">
                v0.9.3 separates interpretation from the executive summary and adds a NextCure-specific relevance layer for targets, side channels, patents, grants, abstracts, and competitive-edge signals.
            </div>
            """,
            unsafe_allow_html=True,
        )
